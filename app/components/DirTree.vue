  ...
  <ul class="list-group w-100 list-group-flush gap-1">
    <li
      ...
      <div class="d-flex w-100 align-items-center text-body list-group-item-action rounded">
        <i @click="toggleDir(entry.path)" :class="{ invisible: entry.type !== 'dir', 'bi-chevron-right': !filesystemStore.expandedDirs.has(entry.path), 'bi-chevron-down': filesystemStore.expandedDirs.has(entry.path) }" class="bi ms-2"></i>
      ...
      <DirTree class="ps-4 pt-1" v-if="entry.type === 'dir' && filesystemStore.expandedDirs.has(entry.path) && entry.children.length" :entries="entry.children"></DirTree>
